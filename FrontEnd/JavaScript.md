...
# Modul 16 (Анимация на JS и CANVAS API)

## Основы анимации и её применение в веб-разработке

В модуле «Анимация и кроссбраузерная вёрстка» вы изучили, как и для чего в веб-разработке используются CSS-анимации, применили такие свойства, как transition и animation, а также познакомились с SVG. В этом модуле мы продолжим погружение в тему анимации, но уже со стороны JavaScript.

**Анимация — это способ создания иллюзии за счёт быстрой смены изображений.**

Иллюзия анимирования строится на физиологическом восприятии человеком быстро сменяющихся изображений (кадров). Единица измерения, которая показывает количество кадров, отображаемых за одну секунду, называется FPS (frames per second). Человек различает 10–12 FPS как отдельные изображения, но когда значение FPS становится больше, уже видно движение.

### Библиотеки для покадровой анимации
[Green Sock](https://greensock.com/) — это библиотека, которая позволяет анимировать всё, что можно получить при помощи JavaScript, включая DOM, Canvas и CSS, а также собственные объекты: например, есть возможность более детально контролировать SVG-анимации и оптимизировать рисование объектов на Canvas.

[Three.js](https://threejs.org/) — ещё одна очень популярная библиотек. Она больше фокусируется на создании и манипулировании 3D-объектами с помощью JavaScript. Three.js часто используется для создания браузерных игр, а также для отрисовки сложных графиков.

## Анимации с помощью JavaScript

*Ключевой аспект покадровой анимации — быстрая смена параметров объектов на каждом кадре. Основным показателем является уже знакомый нам FPS: чем он выше, тем плавнее будет казаться анимация. Достаточной плавности можно достичь при 24 FPS, но эталоном в современном вебе является 60 FPS — этого значения необходимо придерживаться при разработке.*

Что необходимо сделать, чтобы достичь 60 FPS?

Попробуем посчитать, за какое время необходимо отрисовывать один кадр. Для 60 FPS получаем: 1000 миллисекунд / 60 кадров = 16.7 миллисекунд. Довольно мало времени, не так ли? Чтобы в него укладываться, нам необходим высокий уровень знаний и продвинутые инструменты для разработки и отладки.

### ФУНКЦИИ ДЛЯ СОЗДАНИЯ АНИМАЦИИ
Каждый кадр в анимации меняет HTML- и/или CSS-свойства. Это реализуется при помощи функций *setInterval* и *requestAnimationFrame* в JavaScript.

<u>setInterval</u> - Ранее при разработке на JavaScript вы уже сталкивались с этой функцией — давайте вспомним, как она работает. В качестве аргументов setInterval принимает функцию, которую запускает периодически, а также время задержки между этими запусками. Таким образом, setInterval может выполнять некоторый код с определённой периодичностью.
...
// тут будет храниться id созданного интервала
let id = null;

function start() {
  const elem = document.getElementById("block"); 
  
  // позиция элемента в данный момент времени
  let pos = 0;
  
  // запуск выполнения функции frame каждые 20 мс
  id = setInterval(frame, 20);
  
  function frame() {
    if (pos === 350) {
      // как только квадрат достигает 
      // границы родительского элемента,
      // завершаем выполнение интервала
      clearInterval(id);
    } else {
      // если не достигли границы родительского элемента,
      // увеличиваем позицию элемента на 1
      pos += 1; 
...
```
Как можно заметить по исходному коду, движение зелёного квадрата происходит за счёт изменения CSS-параметров top и left. Анимация должна выглядеть плавно при значении 50 FPS, так как используется интервал в 20 миллисекунд.

Вы, вероятно, сталкивались с такой ситуацией: ваш ноутбук или ПК загружен требовательной задачей (запущенная игра, рендеринг видео и тому подобные), и веб-страницы также начинают «тормозить» и своим выполнением дополнительно нагружают устройство. Такого, очевидно, стоит избегать. Но как? К счастью, в JavaScript есть встроенный инструмент для оптимизации перерисовки кадров — функция requestAnimationFrame.

<u>requestAnimationFrame</u> Говоря простым языком, эта функция помогает оптимизировать вызовы других функций, которые отвечают за отрисовку кадров анимации.

Вызов функции выглядит так: `let requestId = requestAnimationFrame(callback)`
где callback — и есть функция отрисовки кадров.

Представим ситуацию, что на веб-сайте есть несколько анимаций, которые выполняются одновременно. Выполнение этих анимаций может начаться в разное время, и тогда браузеру необходимо будет выполнять отрисовку гораздо чаще, чем раз в указанное разработчиком время. Эта ситуация может вызывать заметное падение производительности устройства. Чтобы этого избежать, а также синхронизировать и оптимизировать отрисовку разных анимаций, и используется функция **requestAnimationFrame**.

Также эта функция по умолчанию старается достигать эталонного значения количества кадров (60 FPS), не выполнять перерасчёты лишний раз, например когда вкладка браузера не активна, а также синхронизировать и оптимизировать перерисовки элементов с CSS-анимациями. Таким образом, браузер будет выполнять минимальное количество геометрических перерасчётов.

Как правило, функция callback в **requestAnimationFrame** запускается мгновенно (каждые 10-20 миллисекунд), если CPU не перегружен или нет других факторов, которые могут снижать производительность устройства, например, небольшой процент заряда батареи.

Именно потому, что браузер сам пытается оптимизировать вызовы функции, нельзя задать определённое значение FPS в качестве параметра. Единственное, что возможно сделать, — уменьшить количество кадров путём пропуска некоторых вызовов функции внутри callback.

Теперь вернёмся к предыдущему примеру, где мы анимировали геометрическую фигуру. В этот раз сделаем это при помощи **requestAnimationFrame**. HTML и CSS остаются неизменными — меняется только скрипт на JS:
...
### ИНСТРУМЕНТЫ ОТЛАДКИ
Напоследок немного поговорим об инструментах отладки анимации в браузере — они пригодятся вам в работе.

Если в браузере Google Chrome перейти на вкладку Rendering и включить опцию Frame Rendering Stats (или FPS-meter, если у вас не новейшая версия браузера), то появится счётчик FPS, позволяющий оценить время отрисовки анимации на странице. Также можно воспользоваться вкладкой Performance, где детально отображается, на что браузер тратит время при отрисовке кадра.

## Основы рисования на Canvas
В предыдущем юните мы рассмотрели подход к анимации в JavaScript и научились делать простые анимационные манипуляции. Однако с увеличением сложности анимации и количества объектов, которые в ней участвуют, падает производительность и становится всё сложнее создавать и поддерживать такой код, так как HTML изначально проектировался в первую очередь для статического отображения информации.

Чтобы решить эту проблему, для создания сложных анимаций, рисования и решения других задач, связанных с графикой, используют Canvas.

*Canvas* — это элемент в HTML5, который позволяет работать с растровой 2D-графикой при помощи JavaScript.

<div style="page-break-after: always"></div>

|Плюсы|Минусы|
|---|---|
|Canvas имеет аппаратное ускорение, что очень полезно для работы браузерных видеоигр и сверхсложных анимаций. На практике его часто используют для рисования графиков, обработки изображений, создания анимации и даже рендеринга видео в реальном времени.<br>Также он делает возможным манипулирование к**аждым пикселем**.|Из-за ограничений сборщика мусора невозможно очищать память в любое время в процессе выполнения кода, поэтому есть вероятность высокой нагрузки на процессор и появления недостатка оперативной памяти.<br>Но всего этого можно избежать, если подходить к программированию с умом.|

Важной составляющей Canvas является контекст — часть, которая отвечает за рендеринг содержимого. Таких контекстов с разным содержимым может быть несколько. Для чего может понадобиться несколько контекстов? Во многом это реализовано для удобства разработчиков: например, можно разделить видеоигру или сложный график на несколько «экранов» (состояний), где каждый экран будет отрисовываться на собственном контексте и изменяться в нужный момент.

Также контекст может быть как 2D, так и 3D. 2D-контекст используется по умолчанию и является стандартом, а вот 3D всё ещё поддерживается не всеми браузерами и не стандартизирован.

### РИСОВАНИЕ НА CANVAS
Canvas поддерживает только **одну** примитивную фигуру — прямоугольник. Все остальные фигуры являются производными большого количества наборов точек, соединённых в единую линию, которая называется **path**.

Вот две основные функции, которые используются в Canvas:
* fillRect(x, y, width, height) — рисует **заполненный цветом** прямоугольник по заданным координатам;
* strokeRect(x, y, width, height) — рисует **контуры** прямоугольника по заданным координатам без заполнения цветом изнутри.
...
1. fillRect(x, y, width, height)
  ...
  <html>
    <body>
      ...
      <script>
        ...
        ctx.fillStyle = 'green'; //изменять цвет элемента
        ctx.fillRect(20, 50, 30, 100);
        
        ctx.fillStyle = 'red'; //изменять цвет элемента
        ctx.fillRect(50, 60, 30, 100);
        
        ctx.fillStyle = 'blue'; //изменять цвет элемента
        ctx.fillRect(80, 30, 30, 130);
  ...
  ```
  [fillStyle](https://developer.mozilla.org/ru/docs/Web/API/CanvasRenderingContext2D/fillStyle)

1. strokeRect(x, y, width, height)
  Теперь перейдём к strokeRect и по аналогии с fillRect также создадим столбчатую диаграмму.
  ```html
  <html>
    <body>
      <canvas id="canvas" width="150">
      <script>
        ...
        ctx.strokeRect(40, 50, 30, 100);
        ctx.strokeRect(70, 20, 30, 130);
  ...
  ```
  Как можно заметить, методы fillRect и strokeRect очень похожи. Все остальные фигуры создаются с помощью контуров (path). Давайте рассмотрим, как это делается.

1. **moveTo(x,y)**
  Чтобы отрисовать что-либо на Canvas, используется курсор (перо) — некоторая виртуальная точка, с которой начинается отрисовка любых контуров. Для того чтобы переместить курсор в другое место, используется метод moveTo(x, y), который в качестве аргументов принимает координаты, на которые необходимо переместить курсор.
  Представьте, что вы хотите что-то нарисовать от руки. Для этого вы перемещаете свою руку с карандашом или ручкой в разные места листа бумаги и рисуете — точно так же работает и метод moveTo: он как будто перемещает вашу руку в разные места листа.

1. **lineTo(x,y), stroke(), fill()**
  Метод moveTo практически всегда используется в связке с методом lineTo(x, y). Этот метод очень похож на moveTo, но он не просто перемещает курсор, а добавляет линию из текущего положения курсора в заданную точку, координаты которой передаются в параметрах функции. Почему именно «добавляет», а не «рисует»? Для отрисовки подготовленных примитивов используются другие функции — stroke() и fill(). Отличаются они только тем, что fill заполняет цветом область, которую он будет отрисовывать (по определённым правилам, о которых можно узнать [здесь](https://developer.mozilla.org/ru/docs/Web/API/CanvasRenderingContext2D/fill)), а stroke просто рисует контур цветом.

1. **beginPath(), closePath()**
  Методы beginPath() и closePath() нужны для того, чтобы создавать новые и очищать старые настройки для контуров. Например, после вызова closePath() сбросится положение курсора и цвет для заливки, но эти изменения никак не повлияют на уже отрисованные фигуры.
...
<html>
  <body>
    <canvas id="canvas" width="150">
    <script>
      // получаем объекты canvas и context для дальнейшей работы
      const canvas = document.getElementById('canvas');
      const ctx = canvas.getContext('2d');
      
      // объявляем о начале создания новых контуров
      ctx.beginPath();         
      
      // передвигаем курсор на координаты x = 0, y = 0      
      ctx.moveTo(0,0);

      // рисуем линию из точки с координатами x = 0, y = 0
      // в точку с координатами x = 105, y = 25  
      ctx.lineTo(105,25);


      // рисуем линию из точки с координатами x = 0, y = 0
      // в точку с координатами x = 25, y = 105  
      ctx.lineTo(25,105);
          

      // заливаем получившееся пространство
      // цветом (по умолчанию — чёрный)
      ctx.fill(); 
      
      // завершаем создание контуров
      ctx.closePath();  
    </script>
  </body>
</html>
...
## Создание 2D-анимации на Canvas
Теперь давайте реализуем на Canvas несколько анимаций.
Итак, создадим две основные части для игры Flappy Bird:

Движение фона, чтобы создать иллюзию полёта птицы.
Взмахи крыльев птицы, чтобы придать ей большую пластичность и живость.
Мы должны получить вот такой результат:

![](../img/FRPRO_16_4_1.gif)

### АНИМИРОВАНИЕ ФОНА
Анимировать фон будем следующим способом: для каждого кадра будем синхронно передвигать две части фонового изображения внутри Canvas. По достижении границы Canvas (когда изображение «скроется» за ней) будем каждый раз возобновлять движение.

![](../img/FRPRO_16_4_2.png)

Как можно заметить, тут есть фон, который мы разделим пополам, и несколько изображений птицы в разных состояниях. Итак, у нас есть готовые изображения и алгоритм создания анимации. Отображать изображения на Canvas можно при помощи метода *drawImage()*. Метод drawImage() может принимать в качестве аргумента любой источник изображения, который удовлетворяет нужному формату (SVG, PNG, JPG и многие другие), и отображать его на Canvas. Интересный факт. Как ни странно, drawImage() также может принимать в качестве аргумента видео. Можно подробнее прочитать про это в [документации](https://developer.mozilla.org/ru/docs/Web/API/CanvasRenderingContext2D/drawImage).

Есть несколько вариантов использования drawImage():
* drawImage(image, dx, dy) , где >image — источник изображения, а dx и dy — координаты расположения изображения на Canvas.
* drawImage(image, dx, dy, dWidth, dHeight), где добавляются параметры dWidth и dHeight, которые позволяют явно указать размеры изображения на Canvas.
* drawImage(image, sx, sy, sWidth, sHeight, dx, dy, dWidth, dHeight) — самый продвинутый вариант. Он позволяет установить координаты и размеры изображения (sx, sy, sWidth, sHeight), которое будет получено из источника и отображено с указанными размерами и по указанным координатам (dx, dy, dWidth, dHeight) на Canvas.

Вот отличная иллюстрация того, что и как отображает этот метод:
![](../img/FRPRO_16_4_3.png)

Начнём с базовых файлов HTML и CSS, в которые добавим тег `<canvas>` и минимальную стилизацию:
```html
<!--Содержимое базового файла HTML:-->
<html>
<body>
  <header>
...
```
Теперь приступим к написанию кода на JavaScript — добавим логику получения Canvas и создания контекста для дальнейшей отрисовки изображений:

```js
// Теперь добавим источник изображения, из которого будет брать ассеты:
...
Обратите внимание: появилась функция render, которая будет заниматься отрисовкой каждого кадра. Она будет запускаться после полной загрузки изображения — это нужно, чтобы исключить ситуацию, когда функция начинает работу, а изображение ещё не готово для использования.

Наше изображение пришло в движение — волшебно!

Как это получилось?

1. Добавили функцию requestAnimationFrame, которая вызывает функцию render для отрисовки каждого кадра.
1. Добавили переменную index, которая меняет своё значение каждый кадр. Это нужно для того, чтобы вычислять динамические переменные, которые используются в качестве координат для отображения фона.

Сейчас у вас может возникнуть вопрос: как сделать так, чтобы изображение начало движение заново, после того как достигнет конца Canvas?

Существует несколько способов с определённой периодичностью получать одинаковые числа в результате вычислений. Один из них — вычисление остатка от деления на константу. Константой в нашем случае является ширина Canvas. В итоге после определённого количества итераций в результате вычисления выражения -((index * SPEED) % canvas.width) достигается значение -canvas.width и расчёт начинается заново.

Указываем координаты, по которым хотим получить изображение из источника, и координаты, по которым изображение будет отображено на Canvas, и получаем желаемую анимацию фона.

### АНИМИРОВАНИЕ ДВИЖЕНИЯ ПТИЦЫ
Логика анимирования движения птицы будет немного отличаться от логики анимирования фона.
Если фон нам необходимо было двигать, то в случае с птицей нам достаточно быстро и безостановочно менять три её состояния (взмах крыльев внизу, по центру и вверху). То есть на каждом кадре мы должны менять координаты изображения птицы, которое получаем из источника, но оставлять неизменными координаты изображения на Canvas.

<div style="page-break-after: always"></div>

# Тестирование (Modul 17)

## Качество кода. Зачем нужны тесты

Качество кода — понятие довольно эфемерное. Все о нём знают, но не особо понимают, как его измерить. Например, для оценки и контроля качества менеджмента предприятий применяется ISO 9000, для продуктов — ГОСТ, но вот для качества кода стандарта нет.

На каждом проекте будут свои стандарты, но есть некоторые вещи, необходимость которых подтвердят многие, — например, *code style*.

### Что означает понятие «качество кода»?

Это значит, что код написан по определённым правилам, а также ведётся счёт некоторых метрик, определяющих, насколько код соответствует принятым нормам.

### Зачем нужно следить за качеством кода?
1. Во-первых, подходы в работе и предпочтения всех разработчиков различны. Это может привести к разногласиям. Лучше сразу договориться о том, как вы будете писать код, какие правила использовать и так далее.

1. Во-вторых, текучка на проекте: редкость, когда одни и те же разработчики трудятся на одном проекте от начала до конца. Если не следовать правилам, новый разработчик, приступая к проекту, ничего не поймёт, и кодовая база будет хаотичной.

1. В-третьих, разработчики должны уметь заменять друг друга или приходить на помощь в случае необходимости. Например, может возникнуть ситуация, когда вы с коллегой трудитесь на одном проекте, но на разных доработках и пишете код, не следуя общим правилам. Вам нужно срочно ему помочь, так как он не укладывается в сроки. Тогда порог вхождения будет очень высок и вы потратите гораздо больше времени на разбор, чем потратили бы, если бы ваш с коллегой код был написан в едином стиле.

1. В-четвёртых, если весь код будет выглядеть аккуратно и красиво, вам будет нестыдно говорить о том, что это именно вы его писали :)

### Кто определяет правила и метрики?

* На старте проекта в большинстве случаев их определяют архитектор и ключевой разработчик. Если команда разработчиков небольшая, то соглашения разрабатывают все вместе.
* Когда проект уже развивается, правила могут модифицироваться, метрики — меняться. В этом, как правило, может принимать участие любой из разработчиков.

В принципе, всё зависит от вашей команды и ваших договорённостей.

### По каким критериям создаются правила?
* **Синтаксис**. Правила этой группы определяют, как будет выглядеть код, и не влияют на функционал. Сюда можно отнести правила именования переменных, констант, методов, классов (camelCase, snake_case, PascalCase, UPPER_CASE и другие), количество отступов и переносов строк, (не)использование необязательных символов (например, «висячих» запятых в конце строки многострочного блока кода, фигурных скобок в однострочных выражениях, точек с запятой в конце выражения и так далее).
* **Поддержка кода.** К этой группе можно отнести правила, соблюдение которых облегчает дальнейшую поддержку и изменение кода: грамотное использование паттернов проектирования, дедупликация кода, неиспользование «магических чисел» и «хардкода»
* **Очистка и оптимизация кода.** В эту группу входят правила для рефакторинга: удаление дублей, лишних импортов, переменных и методов, которые уже не используются, но по какой-то причине присутствуют в скрипте.

### КРИТЕРИИ КАЧЕСТВА КОДА
Сейчас мы рассмотрим четыре правила, которые прямо сейчас легко внедрить в разработку, и впоследствии ваши коллеги скажут вам спасибо (и вы сами будете себе за это благодарны). Этот список неполный, но его вполне достаточно для поддержки кода в качественном состоянии.

1. **Наличие code style (Code Style Guide)**

    Code style (кодстайл) — это набор рекомендаций (практик) для написания качественного и красивого кода.

    Они касаются всего, начиная с очень спорных вопросов (что применять — табуляцию или пробел и как использовать точку с запятой) и заканчивая конкретными спецификациями (например, именование функций-конструкторов в JavaScript только с большой буквы).

    Самые популярные Code Style Guides:

    1. [AirBnB](https://github.com/airbnb/javascript)
    1. [Google](https://github.com/google/styleguide)

    Вы уже знакомы с кодстайлом — к нему, например, относится требование по соблюдению name conventions (соглашения об именовании).

2. **Комментирование (документация)**

    Зачастую в начале обучения хочется закомментировать всё и сразу, например чтобы не удалять код, который может пригодиться, или описать своё решение дословно, чтобы никто не ошибся при использовании нового метода.

    *Делать этого категорически не нужно — надо стараться писать такой код, который однозначно будет прочитан другими разработчиками (использовать правильное именование функций и переменных, помещать методы туда, куда нужно в структуре, и т. д.). Но иногда без комментариев просто не обойтись. Например, они могут понадобиться при описании сложного механизма или утилитной функции, которая делает нетривиальное преобразование.*

    Так что же делать?

    Вопрос комментирования — один из самых спорных. К нему нужно подходить очень индивидуально, исходя от специфики компании и продуктов, с которыми она работает: для маленьких проектов комментирование не столь необходимо, для больших же хорошо проработанные правила очень облегчат сопровождение. Поэтому лучше сразу определить случаи,  когда и в каком виде нужно составлять документацию к коду, а когда нет.

1.  **Использование специальных инструментов**

    Выделим насущные проблемы, которые встречаются в разработке, и обсудим инструменты, которые вам помогут в их решении.

    1. Логирование ошибок (Error tracking).

        Вручную писать повсеместно в коде console.log() и выводить ошибки, которые вы ловите в режиме реального времени, неинтересно, утомительно и не всегда правильно.

        Например, вы выложили приложение в продакшен и с ним работают пользователи. Если где-то сломалась консоль или упал сетевой запрос, мы можем это отследить. Error tracking показывает шаги пользователей по DOM-дереву, поддерживает фильтры, имеет гибкую настройку классификации ошибок и т. д.

        Чтобы работать с ошибками было легко и приятно, можно использовать утилиты [Sentry](https://sentry.io/welcome/) и [TrackJS](https://trackjs.com/).

    1. Git workflow

        Если приходится часто проводить много операций через Git, консоль может отнимать много времени. Кроме того, она совсем не наглядна. Есть инструменты, которые помогут быстро справляться с merge-конфликтами, определять зависимости веток, стандартизировать сообщения коммитов. Например, [инструмент для коммитов](https://github.com/commitizen/cz-cli), [инструмент для работы с ветками](https://www.sourcetreeapp.com/).

    1. Форматирование кода

        Форматтеры позволяют держать код в едином стиле, то есть однозначно определять количество пробелов от начала строки, их вид (пробелы или табуляция), расставлять точки с запятой, делать отступы и переносы строк и многое другое. Зачем? Затем, что такой код гораздо проще воспринимать. Например, если писать всё в одну строку, код станет нечитабельным.

        Самый популярный инструмент — [Prettier](https://prettier.io/).

    1. Анализ кода

        Этим занимаются статические анализаторы кода — линтеры. Они помогают приводить код к единому стилю и избегать ошибок, но разница между форматтерами и линтерами огромная.

        Первые форматируют файлы, и неважно, на каком языке вы пишете, — происходит форматирование кода.

        Вторые именно анализируют код —  JS, CSS, Java и др. Например, линтер для CSS, если прогонять его в файле с расширением .js, не принесёт пользы.

        Суть в том, чтобы задать именно синтаксические правила, например, запрет на использование var.

        Также, возможно, вы столкнётесь с таким понятием, как линтинг — это вид статического анализа кода, который часто используют для нахождения проблемных паттернов проектирования или кода, который не следует определённым руководствам по стилю.   

        Самые популярные линтеры: [для JavaScript](https://eslint.org/), [для TypeScript](https://palantir.github.io/tslint/).

1. Покрытие тестами

    В широком смысле тестирование — это процесс проверки соответствия заявленных к продукту требований и реализованной функциональности, осуществляемый путём наблюдения за его работой в искусственно созданных ситуациях и на ограниченном наборе тестов, выбранных определённым образом.

    Иначе говоря, это соответствие реализованного функционала требованиям заказчика.

    Тест — это единица, которая позволяет либо подтвердить, либо опровергнуть работоспособность функционала.

    Несмотря на то что тестирование в этом списке появилось последним, это один из важнейших критериев качества кода. Благодаря покрытию тестами можно наблюдать за тем, какая часть логики протестирована, а какая — нет. Чтобы не упало покрытие при добавлении новой функциональности, нужно будет написать тесты на эту функциональность, что позволит в дальнейшем быть уверенным, что она работает именно так, как было задумано. 

    Благодаря высокоуровневому тестированию можно определить, насколько продукт соответствует ожиданиям, как его части взаимодействуют друг с другом (frontend и backend, например), какими будут реакции продукта на действия пользователя.

    Об этом мы в основном и будем говорить в этом модуле — рассмотрим разные виды тестов и их примеры. Сейчас давайте обсудим количественную характеристику тестирования.

    Уровень покрытия тестами (test coverage) рассчитывается как отношение количества кода, покрытого тестами, к общему количеству кода. Это количество можно измерять по-разному:

    * покрытие файлов — какое количество файлов с кодом затронуто при запуске тестов. Эта метрика используется очень редко, так как файл будет считаться покрытым, даже если тест при запуске использовал всего одну строку из одной функции в этом файле. Таким образом, получится, что мы считаем покрытым тестами и надёжным файл, в котором почти не выполнялся код.
    * покрытие классов — аналогично покрытию файлов, но в качестве единицы кода вместо файлов используют классы. Данная метрика также используется очень редко, так как в одном классе может быть огромное количество функций и такой класс всё равно будет считаться покрытым тестами, если любой из тестов затронет хотя бы одну функцию класса.
    * покрытие методов — примерно то же, что и предыдущий метод, однако в этой метрике не считается, что файл или класс покрыт тестом, если фактически тестом была задета всего одна его функция.
    * покрытие строк — наиболее распространённый метод, так как позволяет определять покрытие не только всех функций, но и всех кейсов (например, всех ветвей условного оператора).
    * покрытие ветвей — ещё один распространённый способ измерения покрытия. Метрика решает ту же проблему, что и предыдущая, только вместо строк считает количество ветвлений алгоритма (например, ветви условного оператора).
    * суммарное покрытие — метрика, использующая сумму различных покрытий. Наиболее употребительный вариант — суммарное покрытие ветвей и строк кода.

## Виды тестирования
**Тестирование** — это одна из техник контроля качества, включающая в себя активности по планированию работ (*Test Management*), проектированию тестов (*Test Design*), выполнению тестирования (*Test Execution*) и анализу полученных результатов (*Test Analysis*).
...
* направленности тестового сценария

и так далее.

**Стадии тестирования**

1. Начнём с самого верхнего уровня — это виды тестирования, соответствующие стадиям разработки программного обеспечения.
    ...
    * Бета-тестирование предполагает всестороннее исследование продукта с целью выявления наибольшего числа ошибок и их последующего устранения. На бета-тестирование могут привлекаться сторонние разработчики и пользователи. Этап бета-тестирования может повторяться после исправления найденных ошибок и багов.

1. По знанию устройства тестируемой системы можно выделить:
    * «чёрный ящик»;
    * «серый ящик»;
    * «белый ящик».

    В данной концепции принято воспринимать тестируемую систему как абстрактный ящик. «Чёрный ящик» предполагает, что при тестировании мы не имеем представления о том, как устроена сама система. Вывод об успешности тестирования делается на основе сравнения соответствия выходных данных ожидаемым.

    Тестирование по стратегии «белого ящика» (называемой иногда стратегией «стеклянного ящика») предполагает полное исследование системы с учётом её внутреннего устройства.
...
1. По направленности тестового сценария принято выделять тестирование позитивных и негативных сценариев. На начальном этапе разработки программного обеспечения основной акцент делается на проверку позитивных сценариев — оценку работоспособности системы в рамках утверждённых бизнес-требований. В дальнейшем осуществляется тестирование негативных сценариев в нестандартных условиях применения, которые потенциально могут возникнуть в процессе эксплуатации.

1. По степени автоматизации можно выделить ручное (manual testing) и автоматизированное (test automation) тестирование. Ручное тестирование осуществляется тестировщиком или разработчиком без использования каких-либо программных средств. Данный подход к тестированию очень трудоёмкий и малоэффективный. Для решения данной проблемы осуществляется автоматизация тестирования — тесты выполняются с помощью программных средств.

## Пирамида тестирования
Существует ключевая концепция, которая позволяет описать структуру автоматизированных тестов для всего приложения, — так называемая пирамида тестирования.

![](../img/m16_pyramid.png)

При написании набора тестов для всего приложения принято руководствоваться принципами, отражёнными в данной пирамиде: необходимо писать тесты для разных уровней детализации и обращать внимание на количество — чем меньше уровень детализации и больше затронутых подсистем, тем меньше тестов.

Пирамида делит тесты на три уровня: юнит-тесты, интеграционные и сквозные тесты.

**Юнит тесты** (unit tests, модульное тестирование). К данной группе относятся тесты, которые предназначены для проверки отдельных частей программы — юнитов (функций, компонентов, модулей). Юнит-тесты обычно пишут разработчики. Такие тесты являются самыми быстродействующими и простыми в реализации.
...
1. Вызов тестируемого метода.
1. Проверка, что возвращаются ожидаемые результаты.

С понятием юнит-тестов связаны два определения — имитации (mocks) и заглушки (stubs). Это так называемые тестовые дублёры (их существует гораздо больше). Иногда эти термины используют в контексте друг друга (иногда они могут быть взаимозаменяемыми). Разница этих понятий в том, что stub не имитирует вообще ничего, то есть это может быть noop (анонимная функция) либо null вместо объекта, когда важен сам факт наличия аргумента . Mocks же несут в себе некоторую информацию. Что именно использовать — нужно решать по факту написания теста, опираясь на его цель.

**Как это работает?**

Реальная функция, класс или модуль заменяется на «подделку», которая для остального кода ничем не отличается от оригинала, но реализована по-другому (чаще всего это функция, которая возвращает заранее предопределённый ответ для использования в юнит-тесте).

Если от функции или класса ожидается какое-то более сложное поведение, такое как изменение ответа со временем, Логику его реализации тоже можно сильно упростить и не включать в неё взаимодействие с внешними системами, с зашитыми данными и так далее, чтобы воспроизводимо запускать тесты.

Примеры инструментов: *Jest, Karma, Mocha*.

**Интеграционные тесты (*Integration tests*).** Чаще всего сложное приложение состоит из многих взаимосвязанных подсистем (различные базы данных, очереди, файловые системы, сеть, серверы, микросервисы, фронтенд и бэкенд и многое другое). Предназначение интеграционных тестов — убедиться в корректной работе всех подсистем как единого целого.

Обычно такие тесты сильно дольше и сложнее юнит-тестов, зато, в отличие от последних, которые закрывают все точки взаимодействия между подсистемами приложения заглушками, интеграционные тесты нацелены именно на проверку таких взаимодействий.

**Как это работает?**

Интеграционные тесты требуют запуска не только собственного приложения, но и компонента, который интегрируется (БД, читаемый с диска файл) .

В пирамиде тестирования интеграционные тесты стоят выше, чем юнит-тесты, поскольку выполнение юнит-тестов с их имитациями, как правило, гораздо быстрее, чем интеграция файловых систем и БД. Также интеграционные тесты писать сложнее, чем небольшие изолированные юнит-тесты. В конце концов, приходится учитывать и работу внешней части теста. Тем не менее, интеграционные тесты имеют преимущество, так как дают уверенность в правильной работе приложения со всеми внешними частями. Юнит-тесты бесполезны для таких задач.

Примеры инструментов: *Cucumber, Protractor, Nightwatch*.

<div style="page-break-after: always"></div>

**Сквозные тесты (End-to-End)** предназначены для проверки приложения в целом. Они охватывают проверку его работоспособности при взаимодействии со всеми вовлёченными системами и фактически отражают то, насколько приложение готово к работе.

Тестовый сценарий тестирования приложения от начала до конца, по сути, повторяет пользовательский сценарий взаимодействия с приложением. Другими словами, такого рода тест проверяет то, как конечный пользователь будет использовать приложение. End-to-end-тест — наиболее полный тест из возможных.

С одной стороны, end-to-end-тест проверяет код наиболее полно в окружении, наиболее приближенном к реальной работе с приложением, но. С другой стороны, такой тест часто довольно сложно разрабатывать и поддерживать: чем комплекснее сам тест и чем сложнее приложение, тем больше возможных сбоев, даже не зависящих от приложения, может случиться (отключение интернета, таймаут ответа от сторонней службы, гонки состояний, особенности браузера и так далее).

Примеры инструментов: *Selenium, Cypress*

## Фреймворки для написания тестов
Как вы уже поняли, есть различные уровни и виды тестирования, и, соответственно, для каждого из них есть свои инструменты. Одним из таких инструментов являются фреймворки. Вспомним, какую функцию они выполняют:

**Фреймворк** — это программное обеспечение, которое упрощает создание и поддержку технически сложных или нагруженных проектов, а также помогает в решении определённых задач. Фреймворк, как правило, содержит только базовые программные модули, а все специфичные для проекта компоненты реализуются разработчиком на их основе. Тем самым достигается не только высокая скорость разработки, но и большая производительность и надёжность решений.

В нашем контексте, фреймворк — это test runner, то есть то, что запускает тесты, а также предоставляет для этого специальные методы вроде "describe", "it", "expect" и другие.

Автоматизация процессов всё прочнее укореняется в нашей жизни. Как результат возрастающего спроса на автоматизацию появились JavaScript-фреймворки для разных целей. Некоторые из них больше подходят для юнит-тестирования, некоторые — для End-to-End.

Нам интересно именно юнит-тестирование, поэтому рассмотрим инструменты для этих тестов. Также мы приведём на будущее некоторые рекомендации по использованию юнит-тестов.

Итак, ниже представлены самые популярные связки инструментов (фреймворк + библиотека, которая расширяет функционал фреймворка собственным API):

* Mocha + Chai (Мока и Чай);
* Karma + Jasmine (Карма и Жасмин);
* Jest + Enzyme (Джест и Энзайм).

### Mocha + Chai

[MochaJS](https://mochajs.org/) — это JavaScript-фреймворк, применяемый для автоматического тестирования приложений. Он может использоваться как на стороне сервера JavaScript, так и в браузере.

[ChaiJS](https://www.chaijs.com/) — это библиотека для node и, как и Mocha, Chai может использоваться на стороне сервера или в браузере. Chai можно применять совместно с любой библиотекой для тестирования.

**Преимущества Mocha**
* гибкое open-source приложение, развитая экосистема;
* легко поддерживает кодогенераторы;
* большое количество туториалов и документов в открытом доступе;
* последовательное выполнение тест-кейсов с гибким составлением отчётов;
* простое сопоставление исключений с соответствующими кейсами.

**Недостатки Mocha**
* не очень хороший инструментарий;
* не лучшая производительность;
* недостаточно полный набор функций для тестирования;
* долгая настройка.

Mocha часто используется в паре с Chai. Chai расширяет функциональность, предоставляя разработчику assert-методы, например should. Также эта библиотека может использоваться с другим фреймворком.

### Karma + Jasmine
Эта пара очень часто используется для тестирования Angular-приложений (Angular — один из фреймворков для создания приложений). Они входят в стандартную сборку angular-cli.

[Karma](https://karma-runner.github.io/latest/index.html) — это целая среда для тестирования, а [Jasmine](https://jasmine.github.io/) чаще всего используется для тестирования асинхронного кода, хоть и имеет много других возможностей. Karma работает на Node.js и составляет точные и гибкие отчёты.
...
* Можно выполнять сценарии с помощью Mocha, Jasmine или другого удобного вам адаптера.
* Исходный код находится в открытом доступе.
* Активное комьюнити.
* Поддерживает инструменты CI/CD**.

**Недостатки Karma**

* Сложно настраивать.

*[Headless-браузер](https://habr.com/ru/company/otus/blog/444248/) — инструмент, с помощью которого можно тестировать код, проверять качество и соответствие отображаемого пользователю продукта макету. Отличие от обычного браузера в том, что у headless-браузера совсем нет UI — всё содержимое он отрисовывает в памяти. За счёт этого он потребляет меньше памяти.

**CI/CD — это процесс непрерывной поставки кода. Когда разработчик заканчивает работу над задачей, она тут же попадает в общий репозиторий и тестовый сайт (либо production), то есть этим функционалом могут начинать пользоваться другие члены команды либо реальные пользователи.
Этот процесс удобен и требует только правильной однократной настройки, что в разы сокращает время работы команды над проектом и позволяет не отвлекаться на какие-то мелочи вроде автоматического запуска тестов после каждого коммита разработчика. Karma поддерживает интеграцию через Jenkins, Semaphore и Travis (это инструменты для настройки CI/CD).

**Преимущества Jasmine**

* совместимость со всеми JS-фреймворками и библиотеками;
* крупное комьюнити со множеством библиотек, туториалов и постов;
* низкий порог вхождения (легко обучиться);
* удобные шаблоны.
...
Jasmine хорошо подходит, если у вас масштабный проект с интеграцией большого количества внешних библиотек. Но если вы используете библиотеку React и ваш проект не такой большой, даже несмотря на то что Jasmine совместим со всеми фреймворками, лучше остановиться на Jest. Особенностью Jasmine является то, что все тесты по умолчанию выполняются последовательно.

### Jest + Enzyme

[Jest](https://jestjs.io/) был разработан Facebook и используется для тестирования JavaScript-кода, особенно React JS-приложений (но также подойдёт и для других фреймворков, например Angular, Vue). Кроме того, Jest можно использовать для тестирования нативного JS-кода.

Особенностью является то, что Jest — не только runner, но также предоставляет asserts (методы для сравнения ожидаемого и фактического результатов) и selectors (методы для поиска элемента) «из коробки» (т. е. может использоваться самостоятельно).

**Преимущества Jest**

* Очень простая настройка. Идёт в комплекте с mock-объектами и библиотекой методов, с помощью которых можно делать утверждения и сравнения (например, равна ли некая переменная определённому значению).
* Хорошая документация.
* Предоставляет надёжный инструментарий.
* Возможность проводить регрессионные тесты*.

**Недостатки Jest**

* Поддерживается не всеми IDE.
* Может быть сложен в освоении.
* Тестирование больших снепшотов (snapshot)** невозможно.

*Регрессионные тесты проводятся, когда были внесены изменения в уже существующий функционал и нужно убедиться, что приложение работает корректно.

Цель регрессионного тестирования — убедиться, что исправление одних дефектов не влечёт возникновение других и что изменения не создали новых дефектов в уже проверенном коде.

**О [snapshot](https://medium.com/@iamsemen/snapshots-%D1%82%D0%B5%D1%81%D1%82%D0%B8%D1%80%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5-react-%D0%BF%D1%80%D0%B8%D0%BB%D0%BE%D0%B6%D0%B5%D0%BD%D0%B8%D0%B9-%D0%B7%D0%B0%D1%87%D0%B5%D0%BC-%D1%8D%D1%82%D0%BE-%D0%BD%D1%83%D0%B6%D0%BD%D0%BE-%D0%B8-%D0%BA%D0%B0%D0%BA-%D1%8D%D1%82%D0%BE-%D0%B8%D1%81%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D1%82%D1%8C-d458cdd3fda1)-тестировании

Ещё одной библиотеки — Enzyme, которая была разработана AirBnB и чаще всего используется также для тестирования React-приложений. С ней стоит подробнее познакомиться на этапе разработки приложений с использованием библиотеки React. В данном случае связка Jest + Enzyme приведена как популярная комбинация тест-раннера и библиотеки.

ВАЖНО: не все библиотеки и фреймворки хорошо комбинируются. Например, комбинация Jest + Chai не имеет смысла, так как Jest внутри себя имеет то, что предоставляет связка Mocha + Chai. То же самое с Jest + Jasmine: Jest написан поверх Jasmine и умеет всё то, что предоставляет эта библиотека. Кроме того, тест-раннеры и библиотеки зачастую написаны под определённый JS-фреймворк/библиотеку, и лучше следовать этим рекомендациям и опираться на опыт большого количества разработчиков. В некоторых фреймворках тест-раннеры и библиотеки заявлены де-факто, например Jest — для React и Karma+Jasmine — для Angular.

## Node.js, NPM, YARN
Пакетные менеджеры упрощают работу с пакетами. В каждом из них есть файл, в котором перечислены все зависимости приложения. При установке зависимостей пакетный менеджер читает этот файл, определяет, какие пакеты нужны зависимостям приложения, собирает единый список зависимостей, скачивает их и устанавливает в систему. Так как основная работа пакетного менеджера — это именно составление так называемого дерева зависимостей и разрешение конфликтов между использованием разных версий одного пакета, пакетные менеджеры также называются менеджерами зависимостей. 

Например, для работы приложения нужен фреймворк Twitter Bootstrap, а он для своей работы использует jQuery. Поэтому если мы укажем менеджеру пакетов установить Twitter Bootstrap, он также автоматически установит и jQuery. 

### Что такое node.js?
Node.js —  это среда выполнения JavaScript вне браузера. С помощью Node.js можно запускать код на сервере. Таким образом, с помощью Node.js на JavaScript можно разрабатывать не только клиентский код, но и консольные утилиты и полноценный бэкенд. Чтобы это стало возможным, Node.js использует движок V8 для интерпретации JavaScript. К тому же Node.js поставляется с массой стандартных библиотек, позволяющих организовать ввод-вывод, работу с файловой системой, сетью и многое другое.

### NPM
NPM (Node.js Package Manager) — предоставляет огромный набор инструментов для установки, удаления, обновления пакетов и так далее. NPM по умолчанию устанавливается вместе с Node.js.

Помимо консольной утилиты для управления зависимостями, NPM предоставляет ещё и открытый репозиторий, позволяющий разработчикам делиться пакетами кода.

Для работы с зависимостями с помощью NPM нам понадобится файл package.json в корне проекта. Внутри этого файла содержится вся информация о разрабатываемом проекте.

**Состав файла package.json:**
...
* зависимости для разработки приложения (в том числе, инструменты).

Данный файл генерируется с помощью команды npm init, которая выводит в консоль несколько вопросов для создания файла. Чтобы не отвечать "yes" на каждый вопрос, необходимо ввести:
...
npm install
```
При запуске этой команды npm будет искать package.json в текущей директории и, если найдёт, установит каждую указанную в нём зависимость.

Это очень удобно, особенно если проект использует много зависимостей (например, jQuery, normalize, browser-cookies и так далее). Если все они прописаны в файле package.json, то для того чтобы другой разработчик смог начать работать с вашим проектом, ему достаточно будет склонировать из Git сам проект, прописать команду npm install, и все указанные зависимости загрузятся автоматически.

В нашем проекте пока только два пакета, но их количество очень быстро возрастает. Установка зависимостей вручную очень неудобна, поэтому для управления ими мы можем использовать файл package.json, расположив его в корне нашего проекта. Сгенерируем его с помощью уже знакомой нам команды npm init. Создастся package.json со всеми зависимостями.

### YARN
YARN — ещё один интересный и удобный менеджер пакетов.
...
1. Yarn кэширует все модули, которые загружает. Вам никогда не придётся загружать один и тот же модуль дважды.
1. Это также означает, что у вас будет удобный доступ к ранее загруженным модулям и без подключения к интернету.
1. Использует чексуммы, чтобы верифицировать каждый устанавливаемый модуль. Это даёт новый уровень безопасности.
1. Использует специальный локфайл в связке с детерминированным алгоритмом установки модулей, что позволяет Yarn гарантировать, что ваше приложение будет одинаково устанавливаться и работать на разных системах (у npm это одно из слабых мест).
...
yarn init -y 
rem Создастся файл package.json

rem Добавьте пакет (зависимость):
yarn add [package]
yarn add [package]@[version]
```

После этого Yarn создаст локфайл yarn.lock в корне вашего проекта. В этот файл будут автоматически записываться чексуммы всех устанавливаемых вами зависимостей. Как говорят сами разработчики, “you don’t need to read or understand this file — just check it into source control”
...
rem Установите все зависимости проекта:
Yarn
rem или
Yarn install
``` 
...
Любая зависимость, установленная хотя бы один раз, попадает в кэш Yarn. Затем эту же зависимость можно установить для другого проекта, не скачивая её ещё раз, то есть использовать её в режиме «офлайн».

```cmd
rem Создадим структуру проекта:
mkdir yarn
cd yarn

rem Создадим файлы package.json и yarn.lock:
Yarn

rem Установим jQuery:
yarn add jquery

rem Установим browser-sync:
yarn add browser-sync –D

rem В результате мы получили файл package.json:
cat package.json {
      “dependencies”: { 
      “jquery”: “^3.1.1” }, 
      “devDependencies”: { 
...
rem Теперь выйдем из проекта и создадим новый:
cd .. 
mkdir yarn-offline 
cd yarn-offline

rem Отключим интернет.

rem Инициализация
Yarn

rem Попробуем установить jQuery:
yarn add jquery 
// warning You don’t appear to have an internet connection. Try the --offline flag to use the cache for registry queries.

rem Следуем рекомендации:
yarn add jquery –offline
 // error Couldn’t find any versions for “jquery” that matches “latest” in our cache. Possible versions: “3.1.1”

 rem Видим, что yarn не знает точно, какая версия jQuery последняя. Предполагает, что это версия 3.1.1. Нас это вполне устраивает:
 yarn add jquery@3.1.1 –offline

rem Убедимся, что всё установлено корректно:
cat package.json
ls node_modules/
```

## End-to-end-тестирование (E2E)
Прежде чем разбираться с тем, что такое end-to-end-тестирование (E2E), стоит вспомнить, какие виды тестов существую:
* Юнит-тесты (unit tests): тестируем отдельные классы, методы или функции.
* Интеграционные тесты (integration tests): тестируем какую-либо интеграцию, чтобы убедиться, что отдельные части целого хорошо работают вместе.
* End-to-end-тесты: тестируем продукт целиком в боевой среде.

Термины выше не взаимоисключающие, они могут пересекаться: E2E-тесты, как правило, интеграционные, ведь редко встречаются односоставные продукты.

В свою очередь, интеграционные тесты могут быть юнит- или функциональными тестами.

Полное и глубокое понимание видов тестов придёт только с практикой, и сейчас нет особого смысла вникать в их детали, различия и пересечения. Сами инструменты для тестирования (библиотеки и продукты) тоже не вносят ясности, ведь один и тот же инструмент может использоваться для разных видов тестов.

На начальном этапе карьеры профессиональное умение писать тесты вам не понадобится. Тем не менее, важно иметь общее представление о задачах, видах и инструментах тестирования.

Настало время разобраться с E2E-тестированием.

Представим, что у нас есть веб-сайт с фронтенд- и бэкенд-частями и какой-то базой данных.

Что делает E2E-тест, проверяющий функцию «логин»? Он открывает браузер, заходит на страницу с этой функцией, вводит (как пользователь) логин и пароль и нажимает на кнопку Log In. Дальше тест проверяет, что, скажем, личный кабинет открывается и доступен.

Таким образом, такой тест затрагивает абсолютно все слои сайта, полностью имитируя пользовательские действия. Этот тест функциональный (функция — «логин»), интеграционный (тестирует взаимодействие фронтенда, бэкенда и базы) и определённо Е2Е, т. к. проверяет работоспособность всех частей сложного продукта от начала до конца.

Как вы могли догадаться из описания выше, E2E-тесты самые сложные, медленные, ресурсоёмкие и дорогостоящие из всех. Такие тесты применяются в крупных сложных проектах, где такой подход к обеспечению качества продукта окупается и имеет смысл для бизнеса.

### Инструменты для E2E-тестирования
Инструментов для E2E много. Самые популярные из них: Selenium, Puppeteer, Playwright, TestCafe, Cypress. Все они решают одну и ту же задачу, применяя немного разные подходы и разный синтаксис.

Ниже представлен график популярности перечисленных инструментов из Google Trends:

![](../img/FRPRO_17_7_1.png)

Видно, что с большим отрывом лидируют Selenium и Cypress. Об их различиях хорошо сказано в этой статье: «Selenium больше ориентирован на специалистов по автоматизации тестирования, а Cypress — на разработчиков для повышения эффективности TDD».. Мы как разработчики будем рассматривать именно Cypress, но в целом, если однажды понять базовые принципы тестирования, потом будет легче освоить любой другой инструмент.

Cypress — один из самых популярных на данный момент инструментов для E2E: многие работодатели указывают его в стеке необходимых для соискателей технологий. Кроме того, Cypress просто установить и использовать, так что он нам отлично подойдёт.
